...
class ActionGenerator:
    ...
    def __init__(self):
        # Initialize Q-table
        self.Q_TABLE_NAME = 'backend/models/Action_Q_Table.npy'
    ...
    def print_q_table(self, q_table, emotion_index, prev_action_index, personality=None, title="Q table"):
        action_names = [ActionStates.index_to_enum(i) for i in range(self.no_of_action_states)]
        headers = ["Prev Action"] + list(action_names)

        table = PrettyTable(headers)
        table.maxwidth = 80

        prev_action_name = ActionStates.index_to_enum(prev_action_index)
    ...
    def generate_action(self, personality_vector, emotional_state_index, previous_action_state_index=ActionStates.Resting) :
        ...
        #print("\n")
        return ActionStates(final_action_state_index)
    ...
    def q_learning(self, personality_vector, current_state, next_action_state):
        ...
        #print("Next Action State:", ActionStates.index_to_enum(next_action_state))

        # Execute action and observe reward
        reward = self.calculate_reward(personality_vector, current_state[1], next_action_state)
        #print("\nReward:", reward)
        ...
        q_current = self.Q[dominant_personality_index][q_range_index][current_state[0].value][current_state[1].value][next_action_state.value]
        max_q_next = np.max(self.Q[dominant_personality_index][q_range_index][current_state[0].value][next_action_state.value])
        ...
        self.Q[dominant_personality_index][q_range_index][current_state[0].value][current_state[1].value][next_action_state.value] = updated_q_value
